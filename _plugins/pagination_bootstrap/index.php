...
function pagination_bootstrap_go($r = array()) 
{
	...
	*/
	...
	if ($out)
	{
		...
		echo NR . '<ul class="pagination pagination-centered">' . $out . '</ul>' . NR;
	}
...
function _pagination_bootstrap($max, $page_number, $base_url, $diappazon = 3, $home_url = '') 
{
	...
	if($page_number == $max)
	{
		$last_link .=  '<li class="disabled"><span>%NEXT%</span></li>';
	} 
	else 
	{
		...
		elseif (($last_mid_link + 1) < $max)
			$last_dots = '<li class="disabled"><span>…</span></li><li><a href="' . $base_url . $max . '">' . $max . '</a></li>';
		else 
			$last_dots = '';  
	}
	...
	if ($page_number == 1)
	{
		$first_link .= '<li class="disabled"><span>%PREV%</span></li>';	
	} 
	else
	{
		if  ($prev_link_page == 1)
		...
		elseif ($first_mid_link > 2)
			$first_dots = '<li><a href="' . $base_url . '1">1</a></li><li class="disabled"><span>…</span></li>';
	}
